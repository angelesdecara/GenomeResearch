! This program is to manage Cebifrons using segment-based coancestry measured 
!
!
program ManageCeb
  implicit none
   integer :: i,j,k,l, icrom, igen, irep, n, iguy,igamete,gametetemp,idummy,io,imarker
   integer, parameter :: dp=KIND(1.d0), nbinsdens=10000
   integer, parameter :: nindbase=7,nmarkers=104035,ncrom=18, ibdsize=500000
   integer, parameter ::  nind=10,nfem0=4, nmale0=1, ngen=10, nrep=1000
   ...
   integer::  pos(nmarkers), ncross, geneal(nind,2),crosses(100), nfem,nmale
   integer :: sol(nind,2),dummy, NumberMarkers(ncrom),nbins
   real(dp):: fibd(nind,nind), recombrate(ncrom), poisson, meandummy
   ...
   real(dp)::oh,gd,fixpos, x, sumx, meanfibd(0:ngen),  densibd(ncrom,nbinsdens+1), meandensibd(0:ngen,ncrom,nbinsdens+1)
   integer :: aux(nind)
   character(len=6)::a
   character(len=2)::base
   character(len=30)::fileHaps,markername, indName,sampleName
   character (len=30):: sizeforfile,formGenCrom
   real:: ranmar
   ...
   formGenCrom='(i7,'//trim(adjustl(formGenCrom))//'f12.8)'
...
!
   ...
   do icrom=1,ncrom
      ...
      fileHaps='chr'//trim(adjustl(fileHaps))//'.phased.haps'
...
! everything 0.d0
   ...
   meanfixedpos =0.d0; varfixedpos=0.d0; densibd=0.d0; meandensibd=0.d0
   meanmaxcontrib=0.d0; meanmincontrib=0.d0; meanfibd = 0.d0
...
! 
      ...
      do icrom=1,ncrom
         do i=1,nbinsdens+1
            meandensibd(0,icrom,i)=meandensibd(0,icrom,i)+densibd(icrom,i)
         enddo
      enddo
...
!
!
      open(54,file='mansegs'//trim(adjustl(sizeforfile))//'kb.dat')
      write(54,'(a130,i4)')'#igen,meanOH,varOH,meanGD,varGD,meanfixedpos,varfixedpos,&
           meanfemalecontrib,varfemalecontrib,meanmalecontrib,varmalecontrib,meanfibd',irep
      do igen=0,ngen
         write(54,'(i4,11f18.14)')igen,meanOH(igen)/dble(irep),varOH(igen)/dble(irep)-(meanOH(igen)/dble(irep))**2,&
              ...
              meanfibd(igen)/dble(irep)
      enddo
      close(54)


      open(55,file='distIBDsegsCeb.dat')
