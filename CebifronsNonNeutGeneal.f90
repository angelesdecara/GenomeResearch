!
! program to manage Cebifrons with genealogical coancestry
! and including deleterious variants
! but not in the calculation of coancestry
! fped
program ManageCebifrons
  implicit none
   integer :: i,j,k,l, icrom, igen, irep, n, iguy,igamete,gametetemp,idummy,imarker, io,jmarker,m,f
   ...
   integer :: genotdummy(nind,nmarkers,2),  genealdummy(nind,2)
   ...
   real(dp):: fped(nind,nind),fpedtemp(nind,nind), recombrate(ncrom), poisson, meandummy, sumx,x
   real(dp):: recomb(ncrom),fpedbase(nind,nind)!=.362726d0 ! cM per Mb
   ...
   real(dp)::meandelfreq(0:ngen),meanhomdel(0:ngen),meanhetdel(0:ngen), ff
...
! everything 0.d0
   ...
   meanmaxcontrib=0.d0; meanmincontrib=0.d0; meanfped=0.d0; meanfitness=0.d0; fitness=0.d0;
...
! 132 -> 5
        if((l.eq.125))then
           if(m.eq.125)fped(1,1)=ff
           if(m.eq.103)fped(1,2)=ff
           if(m.eq.12)fped(1,3)=ff
           if(m.eq.120)fped(1,4)=ff
           if(m.eq.132)fped(1,5)=ff
        endif
        if((l.eq.103))then
           if(m.eq.103)fped(2,2)=ff
           if(m.eq.125)fped(2,1)=ff
           if(m.eq.12)fped(2,3)=ff
           if(m.eq.120)fped(2,4)=ff
           if(m.eq.132)fped(2,5)=ff
        endif
        if((l.eq.12))then
           if(m.eq.12)fped(3,3)=ff
           if(m.eq.103)fped(3,2)=ff
           if(m.eq.125)fped(3,1)=ff
           if(m.eq.120)fped(3,4)=ff
           if(m.eq.132)fped(3,5)=ff
        endif
        if((l.eq.120))then
           if(m.eq.120)fped(4,4)=ff
           if(m.eq.103)fped(4,2)=ff
           if(m.eq.12)fped(4,3)=ff
           if(m.eq.125)fped(4,1)=ff
           if(m.eq.132)fped(4,5)=ff
        endif
        if((l.eq.132))then
           if(m.eq.132)fped(5,5)=ff
           if(m.eq.103)fped(5,2)=ff
           if(m.eq.12)fped(5,3)=ff
           if(m.eq.120)fped(5,4)=ff
           if(m.eq.125)then
              fped(5,1)=ff
              write(6,*)j,k,l,m,ff
           endif
...
!
   ...
   write(6,*)'meanfped bef expansion',sum(fped(1:5,1:5))/25.d0,fped(3,1)
   meandelfreq(0)=0.d0
...
!
      ...
      enddo
      fped=fpedtemp
      write(6,*)'meanfped after expansion',sum(fped(1:nind,1:nind))/dble(nind**2),fped(1,3)
...
!
      ...
      meanfped(0)=meanfped(0)+sum(fped(1:nind,1:nind))/dble(nind**2)
...
! variance in contribs each gen
         ...
         enddo
         meanmalecontrib(igen)=meanmalecontrib(igen)+dble(meandummy)/dble(nmale)
         varmalecontrib(igen)=varmalecontrib(igen)+(dble(meandummy)/dble(nmale))**2
!
         meandummy=0.d0
...
!            write(6,*)idummy,meandummy
         enddo
         meanfemalecontrib(igen)=meanfemalecontrib(igen)+dble(meandummy)/dble(nfem)
         varfemalecontrib(igen)=varfemalecontrib(igen)+(dble(meandummy)/dble(nfem))**2
...
!               write(6,*)'here',idummy,i
         ...
         enddo
         genot=genotdummy
         write(6,*)'attempts seg',idummy,i
!
         do i=1,nind
            fpedtemp(i,i)=.5d0*(1+fped(genealdummy(i,1),genealdummy(i,2)))
            do j=i+1,nind
               fpedtemp(i,j)=.25d0*(fped(genealdummy(i,1),genealdummy(j,1))+&
                    fped(genealdummy(i,1),genealdummy(j,2))+&
                    fped(genealdummy(i,2),genealdummy(j,1))+&
                    fped(genealdummy(i,2),genealdummy(j,2)))
               fpedtemp(j,i)=fpedtemp(i,j)
            enddo
         enddo
         fped=fpedtemp
         ...
         write(6,*)'meanfped',irep,igen,meanfped(igen)
         write(6,*)'contribs',meanmalecontrib(igen)/dble(irep),meanfemalecontrib(igen)/dble(irep)
!         pause
...
!
!
      open(54,file='CebifronsNonNeut/manGen_muk'//trim(adjustl(filename))//'.dat')
      write(54,'(a175,i4)')'#meanOH,varOH,meanGD,varGD,meanfixedpos,varfixedpos,meanfemalecontrib,varfemalecontrib,meanmalecontrib,varmalecontrib,meanfped,meanfitness,meandelfreq,meanhomdel,meanhetdel',irep
      do igen=0,ngen
         write(54,'(i4,15f18.14)')igen,meanOH(igen)/dble(irep),varOH(igen)/dble(irep)-(meanOH(igen)/dble(irep))**2,&
              ...
              meanfped(igen)/dble(irep),meanfitness(igen)/dble(irep),&
