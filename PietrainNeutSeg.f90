...
program ManageSegs
  implicit none
   ...
   integer, parameter :: nindbase=156,nmarkers=49803,ncrom=18, ibdsize=100000
   integer, parameter :: nfem=23, nmale=23, nind=nfem+nmale, ngen=10, nrep=100
...
!         write(6,*)io
         ...
         if(imarker.le.nmarkers)chr(imarker)=icrom
      enddo
      close(53)
      imarker=imarker-1
      if(icrom.gt.1)then
...
!
!
      open(51,file='PietrainNeut/ManSegs'//trim(adjustl(sizeforfile))//'kb.dat')
      write(51,'(a130,i4)')'#igen,meanOH,varOH,meanGD,varGD,meanfixedpos,varfixedpos,&
           meanfemalecontrib,varfemalecontrib,meanmalecontrib,varmalecontrib,meanfibd',irep
      do igen=0,ngen
         write(51,'(i4,11f18.14)')igen,meanOH(igen)/dble(irep),varOH(igen)/dble(irep)-(meanOH(igen)/dble(irep))**2,&
      ...
      open(55,file='PietrainNeut/distIBDsegs.dat')
