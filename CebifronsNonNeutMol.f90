!
! program to manage Cebifrons with molecular coancestry
...
program ManageCebifrons
  implicit none
   ...
   integer::  pos(nmarkers), ncross, geneal(nind,2),nfem,nmale,crosses(0:100),chr(nmarkers)
   integer :: sol(nind,2),dummy, genotbase(nindbase,nmarkers,2),genotBaseHere(nind0,nmarkers,2)
   real(dp):: fmol(nind,nind), recombrate(ncrom), poisson, meandummy, sumx,x
   ...
   real(dp) :: meanmaxcontrib(0:ngen),meanmincontrib(0:ngen),meanfmol(0:ngen)
   ...
   integer :: delposinall(ndelmarkers), cont, aux(nind), locusstart,delseg
...
! everything 0.d0
   ...
   meanmaxcontrib=0.d0; meanmincontrib=0.d0; meanfmol=0.d0; meanfitness=0.d0; fitness=0.d0;
...
!
   selsite=.false.
   open(23,file='CebifronsNonNeutral/delsites')
...
!
      ...
      enddo
         ...
         fmol=0.d0
         call molcoancNoDel(nind,nmarkers,selsite,genot,fmol)

         write(6,*)'averfmol 0',sum(fmol(1:nind,1:nind))/dble(nind**2),sum(fitness)/dble(nind)
...
! variance in contribs each gen
         ...
         write(6,*)'sum contribs',sum(sol(1:nmale,1)),sum(sol(1:nmale,2))
...
!               write(6,*)'here',idummy,i
         ...
         enddo
         genot=genotdummy
         write(6,*)'attempts molman',idummy,i
!
...
!!         call molcoanc(nind,nmarkers,genot,fmol)
         ...
         meanfmol(igen)=meanfmol(igen)+sum(fmol(1:nind,1:nind))/dble(nind**2)
...
!
!
      open(54,file='CebifronsNonNeut/manMolnodel_muk'//trim(adjustl(filename))//'.dat')
      write(54,'(a175,i4)')'#meanOH,varOH,meanGD,varGD,meanfixedpos,varfixedpos,meanfemalecontrib,varfemalecontrib,meanmalecontrib,varmalecontrib,meanfmol,meanfitness,meandelfreq,meanhomdel,meanhetdel',irep
      do igen=0,ngen
         write(54,'(i4,15f18.14)')igen,meanOH(igen)/dble(irep),varOH(igen)/dble(irep)-(meanOH(igen)/dble(irep))**2,&
              ...
              meanfmol(igen)/dble(irep),meanfitness(igen)/dble(irep),&
...
!====================
!
subroutine  molcoancNoDel(nind,nmarkers,selsite,genot,fmol)
!molcoanc(nind,nmarkers,genot,fmol)
  implicit none
  integer, parameter :: dp=KIND(1.d0)
  integer :: i,j,k,l,nind,nmarkers,i1,i2,j1,j2, cont
  integer :: genot(nind,nmarkers,2),ntruemarkers
  real (dp) :: fmol(nind,nind)
  logical :: selsite(nmarkers)

  fmol = 0.d0
  ntruemarkers=count(.not.selsite)
  do i=1,nind
     do j=1,nind
        cont=0
        do i1=1,2
...
!        write(66,*)frohman(i,j)
        enddo
        fmol(i,j)=dble(cont)/dble(4*ntruemarkers)!/dble(4*nmarkers)
     enddo
  enddo

end subroutine molcoancNoDel
!-------------------------------------------
! annealing!
!
