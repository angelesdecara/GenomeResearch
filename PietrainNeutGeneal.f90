! This program is to manage Pietrain using coancestry measured from genealogies averaged over individuals
!
!
program ManagePietrain
  implicit none
   ...
   integer :: pednumber(nind)
   real(dp):: fped(nind,nind), recombrate(ncrom), poisson, meandummy
   ...
   real(dp) :: meanmaxcontrib(0:ngen),meanmincontrib(0:ngen), fpedbase(nind,nind)
   real(dp)::oh,gd,fixpos, x, sumx, fpedtemp(nind,nind)
   integer :: aux(nind)
   character(len=6)::a
   character(len=2)::base
   character(len=30)::fileHaps,markername, indName,sampleName, pedrecode,pedname,comandoi,comandoj
...
!            write(6,*)'sampling guys',sampleName,indName
      ...
      enddo
!
! get their pedigree number
!
      pedname='cosa'
      pedrecode='PI21'//indName(len_trim(indName)-2:len_trim(indName))
    !  write(6,*)'check char',pedrecode
      io=0
!
      open(31,file='PietrainNeut/Genotyped_individual_names_pedigree')
      do while((pedrecode.ne.pedname).and.(io.eq.0))
         read(31,*,IOSTAT=io)pednumber(i),a,pedname
      enddo
      close(31)
      write(6,*)'check pednumb',pedname,pednumber(i)
      if (io.ne.0)  pednumber(i)=0
   enddo
   close(21)
!
! get their coancestries for those in the pedigree
!
   comando='gunzip NonZ*'
   call system('comando')
   comando='cat NonZcoancaa NonZcoancab > NonZcoanc.txt'
   call system('comando')
   comando('gzip NonZ*')
   call system('comando')
   fped=0.d0
   do i=1,nind
      fped(i,i)=.5d0
      do j=i+1,nind
         if((pednumber(i).ne.0).and.(pednumber(j).ne.0))then
            write(comandoi,*)pednumber(i)
            write(comandoj,*)pednumber(j)
            comando='zgrep '//trim(adjustl(comandoi))//' PietrainNeut/NonZcoanc.txt.gz | grep '//trim(adjustl(comandoj))//' > ftemp1'
            write(6,*)comando
            call system(comando)
            open(32,file='ftemp1')
            read(32,*)idummy,idummy,idummy,idummy,fped(i,j)
            fped(j,i)=fped(i,j)
            if(fped(i,j).gt.1.d0)then
               write(6,*)'fped>1',fped(i,j)
               stop
            end if
         endif
         close(32)
         call system("rm ftemp1")
            ...
            do i=1,nmale
               if(sol(i,n).gt.0)then
                  do j=1,sol(i,n)
                     dummy=dummy+1
                     aux(dummy)=i
                  !   geneal(dummy,1)=i ! geneal(i,1) is father of dummy
                  enddo
               endif
            enddo
         enddo
...
!
                  ...
                  do l=1,ncross
                     crosses(l)=ranmar()*(sum(NumberMarkers(1:icrom))-sum(NumberMarkers(1:icrom-1)))+1+sum(NumberMarkers(1:icrom-1))
...
!
!
      open(54,file='PietrainNeut/manGenPI.dat')
